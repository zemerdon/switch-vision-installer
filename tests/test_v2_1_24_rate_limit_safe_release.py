...
class InstallerRateLimitSafeReleaseTests(unittest.TestCase):
    ...
    def test_component_manager_core_changelog_is_raw_repository_text(self):
        core = next(spec for spec in component_manager.COMPONENTS if spec.component_id == "core")
        with patch.object(component_manager, "resolve_repository", return_value="switch-vision-releases"), \
             patch.object(component_manager, "_raw_text", return_value="# Changelog\n") as raw, \
             patch.object(component_manager, "_component_status", return_value={"latest_version": "2.4.3", "installed_version": "2.4.2"}), \
             patch.object(component_manager, "_github_request", side_effect=AssertionError("REST API must not be used")):
            payload = component_manager.component_changelog(core.component_id)
        self.assertEqual(payload["changelog"], "# Changelog\n")
        raw.assert_called_once_with("switch-vision-releases", "CHANGELOG.md")
